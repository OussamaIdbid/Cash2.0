_dxBlazorInternal.define('cjs-scheduler-329eb609.js', function(require, module, exports) {'use strict';
...
function togglePositionInCaseOfUndefinedHintSizes(targetElement, hintElement) {
    var x = domUtils.GetAbsoluteX(targetElement);
    var y = domUtils.GetAbsoluteY(targetElement);
    domUtils.SetAbsoluteX(hintElement, x);
    domUtils.SetAbsoluteY(hintElement, y);
}
var RenderHelper = (function () {
    ...
    function ensureCalloutPosition(mainElement, prevPosition, currentPosition) {
        ...
        domUtils.RemoveClassNameFromElement(mainElement, prevClassName);
        domUtils.AddClassNameToElement(mainElement, currentClassName);
    }
...
function PositionCalcContext(targetElement, hintElement, options) {
    ...
    function SizeContent(targetElement, hintElement, options, calloutSize, isX) {
        ...
        this.screen.min = isX ? domUtils.GetDocumentScrollLeft() : domUtils.GetDocumentScrollTop();
        var viewPortWidth = __chunk_1.Browser.WebKitTouchUI ? window.innerWidth : domUtils.GetDocumentClientWidth();
        this.screen.max = this.screen.min + (isX ? viewPortWidth : domUtils.GetDocumentClientHeight());
        this.target = { min: 0, max: 0 };
        this.target.min = isX ? domUtils.GetAbsoluteX(targetElement) : domUtils.GetAbsoluteY(targetElement);
...
var PositionHelper = (function () {
    ...
    function set(targetElement, windowElement, options) {
        ...
        var x = typeof options["x"] !== "undefined" ? options.x : domUtils.GetAbsoluteX(targetElement) + pos.location.x + customOffsets.x;
        var y = typeof options["y"] !== "undefined" ? options.y : domUtils.GetAbsoluteY(targetElement) + pos.location.y + customOffsets.y;
        domUtils.SetAbsoluteX(windowElement, x);
        domUtils.SetAbsoluteY(windowElement, y);
        return {
...
function Init(scheduler, dropDownDateNavigatorElement, appointmentToolTipElement, appointmentEditForm, dotnetHelper) {
    if(scheduler) {
        if(scheduler.appointmentToolTipElement)
            { __chunk_2.DisposeEvents(scheduler.appointmentToolTipElement); }
        if(scheduler.dropDownDateNavigatorElement)
            { __chunk_2.DisposeEvents(scheduler.dropDownDateNavigatorElement); }
    }
    scheduler = domUtils.EnsureElement(scheduler);
    if(!scheduler) { return; }
    scheduler.dropDownDateNavigatorElement = domUtils.EnsureElement(dropDownDateNavigatorElement);
    scheduler.appointmentToolTipElement = domUtils.EnsureElement(appointmentToolTipElement);
    scheduler.appointmentEditForm = domUtils.EnsureElement(appointmentEditForm);
    ...
    if(scheduler.skipCalculateAllAppointments) {
        var filter = function(apts) {
            ...
            for(var i = 0, apt; apt = apts[i]; i++) {
                if(domUtils.ElementHasCssClass(apt, "dxbs-apt-edited"))
                    { result.push(apt); }
            }
            return result;
        };
    ...
    function initEvents(scheduler, dropDownDateNavigatorElement, appointmentToolTipElement, dotnetHelper) {
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "mousedown", mouseDownEventHandler);
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "mouseup", mouseUpEventHandler);
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "mousemove", mouseMoveEventHandler);
        __chunk_3.LongTabEventHelper.attachLongTabEventToElement(scheduler, getTouchLongPressEventHandler);
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "touchstart", touchStartEventHandler);
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "touchmove", touchMoveEventHandler);
        __chunk_3.LongTabEventHelper.attachEventToElement(scheduler, "touchend", touchEndEventHandler);
        if (dropDownDateNavigatorElement) {
            ...
            domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseDownEventName, dropDownDateNavigatorElement.dateNavigatorLostFocusHandler);
            __chunk_2.RegisterDisposableEvents(dropDownDateNavigatorElement, function () {
                domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseDownEventName, dropDownDateNavigatorElement.dateNavigatorLostFocusHandler);
            });
        }
        if (appointmentToolTipElement) {
            ...
            domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseDownEventName, appointmentToolTipElement.toolTipLostFocusHandler);
            __chunk_2.RegisterDisposableEvents(appointmentToolTipElement, function () {
                domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseDownEventName, appointmentToolTipElement.toolTipLostFocusHandler);
            });
        }
        domUtils.subscribeElementContentWidth(scheduler, adjustControl);
    }
    ...
    function mouseDownEventHandler(e) {
        if(__chunk_3.hasUnforcedFunction("TouchStart") || e.button === 2) {
            if(e.button === 2)
        ...
        if (apt) {
            ...
            if (isTopHandleElement(e.srcElement) || isLeftHandleElement(e.srcElement))
                ...
                { __chunk_3.unforcedFunctionCall(function() { scheduler.mouseMoveHandlerState = MouseMoveHandlerState.Drag; }, "drag", 50, true); }
        }
    ...
    function mouseMoveEventHandler(e) {
        ...
        if (!scheduler.throttledDrag) {
            scheduler.throttledDrag = __chunk_6.Timer.Throttle(function (e) {
    ...
    function mouseUpEventHandler(e) {
        if(__chunk_3.hasUnforcedFunction("TouchEnd") || e.button === 2) {
            if(e.button === 2)
                { domUtils.PreventEventAndBubble(e); }
            return;
        }
        ...
        else if(DomUtils.getAppointmentContainer(e.srcElement) && !__chunk_3.hasUnforcedFunction("skipToolTip")) {
            dotnetHelper.invokeMethodAsync('ShowAppointmentToolTip');
            __chunk_3.unforcedFunctionCall(function() {}, "skipToolTip", 300);
        }
        __chunk_3.clearUnforcedFunctionByKey("drag");
        scheduler.mouseMoveHandlerState = MouseMoveHandlerState.None;
    }
    ...
    function touchEndEventHandler(e) {
        ...
        __chunk_3.unforcedFunctionCall(function() { }, "TouchEnd", 300, true);
    }
    return Promise.resolve("ok");
}
function getToolTipOptions(scheduler, toolTipElement, apt) {
    ...
    if(domUtils.ElementHasCssClass(toolTipElement, "dxsc-tooltip")) {
...
var DomUtils = (function () {
    ...
    function getAppointmentContainer(el) {
        return domUtils.GetParentByClassName(el, "dxbs-sc-apt");
    }
...
function isTopHandleElement(el) {
    return domUtils.ElementHasCssClass(el, "dxsc-top-handle");
}
function isBottomHandleElement(el) {
    return domUtils.ElementHasCssClass(el, "dxsc-bottom-handle");
}
function isLeftHandleElement(el) {
    return domUtils.ElementHasCssClass(el, "dxsc-left-handle");
}
function isRightHandleElement(el) {
    return domUtils.ElementHasCssClass(el, "dxsc-right-handle");
}
function isTimeCellElement(el) {
    return domUtils.ElementHasCssClass(el, "dxbs-sc-all-day-area") || domUtils.ElementHasCssClass(el, "dxbs-sc-time-cell");
}
function isAllDayCell(el) {
    return domUtils.ElementHasCssClass(el, "dxbs-sc-all-day-area");
}
function isAppointmentSelected(el) {
    return domUtils.ElementHasCssClass(el, "dxbs-apt-selected");
}
...
function Dispose(scheduler) {
    scheduler = domUtils.EnsureElement(scheduler);
    if (scheduler)
        { __chunk_2.DisposeEvents(scheduler); }
    return Promise.resolve("ok");
}
var scheduler = { Init: Init, Dispose: Dispose };

exports.Dispose = Dispose;
exports.Init = Init;
exports.default = scheduler;},["cjs-chunk-c43b3f7c.js","cjs-dom-utils-6a09eac3.js","cjs-chunk-509db829.js","cjs-chunk-9109506d.js","cjs-chunk-85e9b555.js"]);
