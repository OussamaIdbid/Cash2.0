_dxBlazorInternal.define('cjs-grid-b52f8083.js', function(require, module, exports) {'use strict';
...
function MouseDown(evt, dragId, dotnetHelper) {
    ...
    var mouseUp = function () {
        domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseMoveEventName, mouseMove);
        domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseUpEventName, mouseUp);
        if (!dragWasActivated && evtTarget && __chunk_1.Browser.WebKitTouchUI)
            { evtTarget.click(); }
    };
    domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseMoveEventName, mouseMove);
    domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseUpEventName, mouseUp);
    evt.preventDefault();
    return false;
}
function DragStart(evt, evtTarget, dragId, dotnetHelper) {
    var headSelector = GetGraggableSelector(dragId);
    var dragElementOrigin = domUtils.findParentBySelector(evtTarget, headSelector);
    ...
    var startDocScrollLeft = domUtils.getDocumentScrollLeft();
    var startDocScrollTop = domUtils.getDocumentScrollTop();
    ...
    var mouseUp = function () {
        ...
        domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseMoveEventName, mouseMove);
        domUtils.DetachEventFromElement(document, __chunk_3.TouchUIHelper.touchMouseUpEventName, mouseUp);
        domUtils.DetachEventFromElement(window, "scroll", scroll);
        dragElement.parentNode.removeChild(dragElement);
    };
    var scroll = function () {
        docScrollDiff = {
            left: startDocScrollLeft - domUtils.getDocumentScrollLeft(),
            top: startDocScrollTop - domUtils.getDocumentScrollTop()
        };
    };
    domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseMoveEventName, mouseMove);
    domUtils.AttachEventToElement(document, __chunk_3.TouchUIHelper.touchMouseUpEventName, mouseUp);
    domUtils.AttachEventToElement(window, "scroll", scroll);
}
...
function target(element, x, top, bottom, columnVisibleIndex, groupVisibleIndex, columnHeadType, insertBefore, wholeRowIsRarget) {
    ...
    this.docScrollTop = domUtils.getDocumentScrollTop();
    this.docScrollLeft = domUtils.getDocumentScrollLeft();
}
...
function activateTarget(target, dragId) {
    ...
    targetMark.style.top = target.top + (target.docScrollTop - domUtils.getDocumentScrollTop()) + domUtils.getDocumentScrollTop() - 1 - padding + "px";
    targetMark.style.height = (target.bottom - target.top) + 2 * padding + "px";
    targetMark.style.left = target.x + (target.docScrollLeft - domUtils.getDocumentScrollLeft()) + domUtils.getDocumentScrollLeft() + "px";
...
function ScrollToSelectedItem(mainElement) {
    if(!domUtils.elementIsInDOM(mainElement)) { return; }
...
function getGridSelectedRowElement(dropDownElement) {
    ...
    domUtils.QuerySelectorFromRoot(dropDownElement, function(rootQuery) {
        glLbElement = dropDownElement.querySelector("*"+ rootQuery + " > *[id$='_LB']");
    });
    if(!glLbElement) {
        domUtils.QuerySelectorFromRoot(dropDownElement.parentNode, function(rootQuery) {
...
function InitScrollbarWidth(scrollElement, scrollHeaderElement, options) {
    ...
    if(HasVerticalScrollBar(scrollElement, options)) {
        ...
        var scrollBarWidth = __chunk_5.GetVerticalScrollBarWidth();
...
function AutoWidthCalculate(mainElement, scrollElement, scrollHeaderElement, virtualScrollSpacerTopElement, virtualScrollSpacerBottomElement, options) {
    ...
    if(scrollHeaderElement) {
        var correction = HasVerticalScrollBar(scrollElement, options) ? __chunk_5.GetVerticalScrollBarWidth() : 0;
        if(width)
...
function InitGroupButtonCellWidth(mainElement) {
    ...
    for(var i = 0; i < groupColumnHeadButtons.length; i++) {
        ...
        var buttonWidth = groupColumnHeadButton.offsetWidth + domUtils.GetLeftRightBordersAndPaddingsSummaryValue(groupColumnHeadButton.parentNode);
...
function Init(mainElement, scrollElement, scrollHeaderElement, virtualScrollSpacerTopElement, virtualScrollSpacerBottomElement, options, dotnetHelper, scrollToSelectedItemRequested) {
    ...
    scrollElement = domUtils.EnsureElement(scrollElement);
    if(scrollElement) {
        scrollHeaderElement= domUtils.EnsureElement(scrollHeaderElement);
        scrollElement.dataset.InScrollRequest = true;
        if(options.isVirtualScrolling) {
            virtualScrollSpacerTopElement = domUtils.EnsureElement(virtualScrollSpacerTopElement);
            virtualScrollSpacerBottomElement = domUtils.EnsureElement(virtualScrollSpacerBottomElement);
        }
        ...
        domUtils.AttachEventToElement(scrollElement, "scroll", onScroll);
        if(AutoWidthRequired(options)) {
            ...
            onWindowResize();
            domUtils.AttachEventToElement(window, "resize", onWindowResize);
        }
    }
    ...
    if(options.isColumnDragEnabled) {
        ...
        if(dragId) {
            ...
            if(columnHeadRow) {
                ...
                domUtils.AttachEventToElement(columnHeadRow, __chunk_3.TouchUIHelper.touchMouseDownEventName, columnHeadRowMouseDown);
            }
            ...
            if(groupPanel) {
                ...
                domUtils.AttachEventToElement(groupPanel, __chunk_3.TouchUIHelper.touchMouseDownEventName, groupRowMouseDown);
            }
        }
    }
    __chunk_2.RegisterDisposableEvents(mainElement, function () {
        if(onScroll)
            { domUtils.DetachEventFromElement(scrollElement, "scroll", onScroll); }
        if(onWindowResize)
            { domUtils.DetachEventFromElement(window, "resize", onWindowResize); }
        if(scrollElement)
            { ClearVirtualScrollingTimer(scrollElement); }
        if(columnHeadRowMouseDown && columnHeadRow)
            { domUtils.DetachEventFromElement(columnHeadRow, __chunk_3.TouchUIHelper.touchMouseDownEventName, columnHeadRowMouseDown); }
        if(groupRowMouseDown && groupPanel)
            { domUtils.DetachEventFromElement(groupPanel, __chunk_3.TouchUIHelper.touchMouseDownEventName, groupRowMouseDown); }
    });
...
var grid = { Init: Init, Dispose: Dispose };

exports.Dispose = Dispose;
exports.GetParametersForVirtualScrollingRequest = GetParametersForVirtualScrollingRequest;
exports.HasParametersForVirtualScrollingRequest = HasParametersForVirtualScrollingRequest;
exports.Init = Init;
exports.ScrollToSelectedItem = ScrollToSelectedItem;
exports.default = grid;},["cjs-chunk-c43b3f7c.js","cjs-dom-utils-6a09eac3.js","cjs-chunk-509db829.js","cjs-chunk-9109506d.js","cjs-chunk-7aa0d757.js","cjs-chunk-e26655d2.js"]);
