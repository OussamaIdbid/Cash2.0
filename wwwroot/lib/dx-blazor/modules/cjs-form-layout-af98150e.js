_dxBlazorInternal.define('cjs-form-layout-af98150e.js', function(require, module, exports) {'use strict';
...
function createCssRuleForScreenSizeMediaQuery(cssVarName, fallbackWidth, cssRule) {
    ...
    var minWidth = cache[cssVarName] || (cache[cssVarName] = (domUtils.GetCurrentStyle(document.body).getPropertyValue(cssVarName) || fallbackWidth));
    if (minWidth) {
        var styleSheet = domUtils.GetCurrentStyleSheet();
        if (styleSheet)
...
function createAdaptivityCssRules(mainElementId) {
    ...
    var width = 0;
    domUtils.AddClassNameToElement(mainElement, CALC_CLASS);
    ...
    domUtils.RemoveClassNameFromElement(mainElement, CALC_CLASS);
}
function getFilteredElements(mainElementId, elements) {
    var mainElement = GetMainElement(mainElementId);
    return domUtils.RetrieveByPredicate(elements, function (el) { return domUtils.GetParentByClassName(el, FL_CLASS) == mainElement; });
}
function getMaxCaptionWidth(mainElementId, container, recursive) {
    ...
    return width > 0 ? (width + (__chunk_1.Browser.HardwareAcceleration ? 1 : 0)) : 0;
}
function getAdaptivityCssRulesPrefix(mainElementId, container) {
    ...
    while (true) {
        flElement = domUtils.GetParentByClassName(flElement.parentNode, FL_CLASS);
        if (flElement)
...
var formLayout = { Init: Init };

exports.Init = Init;
exports.default = formLayout;},["cjs-chunk-c43b3f7c.js","cjs-dom-utils-6a09eac3.js"]);
